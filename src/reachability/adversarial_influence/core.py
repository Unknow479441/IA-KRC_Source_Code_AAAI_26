...
class AdversarialInfluenceMap:
    ...
    def get_move_amount(self) -> float:
        ...
        return self.opponent_model.config.get('move_amount',
                                              2)  # move_amount passed from sc2custom.yaml, default value changed to 2
    ...
    def query_adversarial_reachability(self, start_pos: Tuple[float, float],
        ...
        if agent_id and str(agent_id) in self.reachability_maps:
            ...
            if not reachability_map[goal_x, goal_y]:  # Fix: Use goal_x, goal_y
                print(
                    f"[Reachability Query] Target ({goal_x}, {goal_y}) is not reachable for Agent {agent_id} (k_step={current_k_step})")
                return False, float('inf'), []
        ...
        print(
            f"[Reachability Query] Pathfinding result: reachable={is_reachable}, cost={cost:.2f}, path_length={len(path)} (k_step={current_k_step})")
    ...
    def integrate_with_base_costs(self, base_cost_map: np.ndarray) -> np.ndarray:
        ...
        if base_cost_map.shape != (self.height, self.width):
            logger.warning(
                f"Base cost map size mismatch: expected {(self.height, self.width)}, got {base_cost_map.shape}")
    ...
    def get_heatmap_for_visualization(self, map_type: str = 'influence') -> np.ndarray:
        ...
        if map_type == 'influence':
            ...
            return self.enemy_influence_map if self.enemy_influence_map is not None else np.zeros(
